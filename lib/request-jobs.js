        ...
        if(status.code === 200) {
          ...
          for(let exchange of exchanges) {
            ...
            while(hasResults) {
              ...
              if(typeof exchangesTickers !== 'undefined' && 'tickers' in exchangesTickers.data && exchangesTickers.data.tickers.length) {
                let parsedExchangeTickers = exchangesTickers.data.tickers.map(ticker => {
                  return {
                    exchangeId: ticker.market.identifier,
                    assetSymbol: 'base' in ticker ? ticker.base.toLowerCase() : '',
                    quoteSymbol: 'target' in ticker ? ticker.target.toLowerCase() : '',
                    isStale: ticker.is_stale
                  }
                })
                console.log(`${exchange.api_id}: ${exchangesTickers.data.tickers.length}`)
