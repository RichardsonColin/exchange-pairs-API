...
const Bottleneck = require('./helpers/Bottleneck');
...
class CoinGecko {
  ...
  get exchanges() {
    ...
    return {
       ...
       * @param {object} params - Parameters to pass through to the request
       * @param {number} params.page - Page through results
       * @param {number} params.per_page - Amount of results per page (default: 100; min: 1: max: 250;)
       * @returns {ReturnObject}
       */
      all: (params = {}) => {
        const path = `/${pathPrefix}`;
  ...
  async _request(path, params) {
    ...
    return await this._limit(requestPromise);
  }

  /**
   * @description Sets limit on https request
   * @function _limit
   * @protected
   * @param {funtion} requestPromise - http request promise
   * @returns {Promise} Queued http request
   */

  async _limit(requestPromise) {
    // Creates an instance of the Bottleneck class
    if (this.Limiter === undefined) {
      this.Limiter = Bottleneck.Init(
        Constants.LIMIT.BATCH_TOTAL,
        Constants.LIMIT.BATCH_REFRESH_AMOUNT,
        Constants.LIMIT.BATCH_REFRESH_INTERVAL,
        Constants.LIMIT.CONCURRENT_REQUESTS,
        Constants.LIMIT.TIME_BETWEEN_REQUESTS
      );
    }

    return await Bottleneck.AddToQueue(requestPromise, this.Limiter);
  }
}
